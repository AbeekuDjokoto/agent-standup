...
function summarize(entries) {
  ...
  const totalLoanValue = entries.reduce(
    (sum, entry) => sum + entry.totalLoanValue,
    0,
  );
...
function normalizePayload(payload = {}) {
  ...
  if (!Number.isFinite(numberOfApplications) || numberOfApplications < 0) {
    throw new Error(
      'Number of applications must be a valid non-negative number.',
    );
  }
  if (!Number.isFinite(totalLoanValue) || totalLoanValue < 0) {
    throw new Error(
      'Total value of loans must be a valid non-negative number.',
    );
  }
...
app.listen(port, () => {
  console.log(`Loan activity backend running on http://localhost:${port}`);
});
